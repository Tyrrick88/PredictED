...
- Deploy the backend to Railway from the repo root. `railway.json` points Railway at the root `Dockerfile`, which builds the Spring Boot app from `backend` on Java 21.
- Deploy the frontend to Vercel from the `frontend` root. The build defaults to `https://predicted-production-3c23.up.railway.app`; set `PREDICTED_API_BASE` in Vercel only if the Railway backend URL changes.
